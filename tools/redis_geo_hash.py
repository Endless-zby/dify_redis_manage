...
class RedisGeoHashTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        """
        Get the geohash of geospatial items
        """
        # 获取参数
        key = tool_parameters.get("key")
        members_str = tool_parameters.get("members")
        logger.info(f"获取Redis中地理位置项的geohash：{key}")
        ...
        try:
            ...
            # 获取地理位置项的geohash
            hashes = redis_client.geohash(key, *members)
            
            # 关闭连接
            redis_client.close()
            
            # 返回结果
            yield self.create_variable_message("hashes", hashes)
            
        except Exception as e:
            yield self.create_text_message(f"Error getting geohashes from Redis: {str(e)}")