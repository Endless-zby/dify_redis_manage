...
class RedisStringTtlTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        logger.info(f"获取Redis键生存时间开始：{key}")
        ...
        try:
            ...
            # 返回结果
            yield self.create_variable_message("ttl", ttl)
            
        except Exception as e:
            yield self.create_text_message(f"Error getting TTL from Redis: {str(e)}")