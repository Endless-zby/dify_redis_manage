...
class RedisManageTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        logger.info(f"获取Redis值开始：{key}")
        # 参数验证
        if not key:
            yield self.create_text_message("Error: Key is required.")
            logger.info("Error: Key is required.")
            return
        ...
        try:
            ...
            logger.info(f"获取Redis")
            # 从Redis获取值
            conform = redis_client.exists(key)
            logger.info(f"dddd: {conform}")
            value = redis_client.get(key)
            logger.info(f"ssss: {value}")
            # 关闭连接
            redis_client.close()
