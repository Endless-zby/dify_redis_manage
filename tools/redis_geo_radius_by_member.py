...
class RedisGeoRadiusByMemberTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        """
        Get geospatial items within a radius from a member
        """
        # 获取参数
        ...
        logger.info(f"在Redis中搜索指定成员半径范围内的地理位置项：{key}")
        ...
        try:
            ...
            # 获取指定成员半径范围内的地理位置项
            members = redis_client.georadiusbymember(key, member, radius, unit=unit)
